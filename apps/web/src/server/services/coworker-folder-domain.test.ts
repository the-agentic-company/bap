...
function coworkerRow(
  ...
  return {
    ...
    createdByAvatarSnapshot: input.createdByAvatarSnapshot ?? null,
    ...
    automationOwnerUserId: input.automationOwnerUserId ?? input.ownerId,
    automationOwnerConsentedAt: input.automationOwnerConsentedAt ?? null,
    proposedAutomationOwnerUserId: input.proposedAutomationOwnerUserId ?? null,
    proposedAutomationOwnerAt: input.proposedAutomationOwnerAt ?? null,
    configurationRevision: input.configurationRevision ?? 0,
    createdAt: input.createdAt ?? now,
    updatedAt: input.updatedAt ?? now,
  };
}
